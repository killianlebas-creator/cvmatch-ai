...
const s = {
  ...
  grid: (cols, gap=24) => ({ display:"grid", gridTemplateColumns:cols, gap }),
  ...
  btn: (variant="primary", size="md") => ({
    ...
    fontSize: size==="lg" ? "1rem" : size==="sm" ? ".8rem" : ".9rem",
    fontWeight:600, border:"none", textDecoration:"none", transition:"all .2s ease",
    cursor:"pointer", fontFamily:"var(--fb)",
    ...(variant==="primary" ? {
      background:"var(--accent)", color:"white",
      boxShadow:"0 4px 14px rgba(26,107,74,.25)"
    } : variant==="outline" ? {
  ...
  // Typography
  display: (size="2.6rem") => ({
    fontFamily:"var(--fd)", fontSize:size,
  ...
  textarea: {
    ...
    border:"1.5px solid var(--border)", borderRadius:"var(--r)",
    fontSize:".875rem", color:"var(--ink)",
    background:"white", outline:"none", resize:"vertical",
    lineHeight:1.65, fontFamily:"var(--fb)", transition:"border .2s"
  }
};
...
const Icon = ({ name, size=16, color="currentColor" }) => {
  const icons = {
    ...
    history:  <><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.97"/></>,
    ...
    sparkle:  <><path d="M12 3L9.5 8.5 3 9l5 4.5L6.5 20 12 17l5.5 3L16 13.5 21 9l-6.5-.5L12 3z"/></>,
...
/* ─────────────────────────────────────────────
   FAKE AUTH STORE (in-memory)
───────────────────────────────────────────── */
const DEMO_USER = {
  name: "Marie Laurent", email: "marie@exemple.fr",
  plan: "pro", trialDaysLeft: 5,
  stats: { optimized: 12, avgScore: 78, bestScore: 94 }
};

const DEMO_HISTORY = [
  { id:1, company:"Spotify", role:"Chef de Projet Digital", date:"2025-03-20", scoreBefore:22, scoreAfter:89, keywords:["Agile","Scrum","KPI","OKR","Jira"] },
  { id:2, company:"BNP Paribas", role:"Responsable Marketing", date:"2025-03-17", scoreBefore:35, scoreAfter:82, keywords:["CRM","ROI","Salesforce","Lead Gen"] },
  { id:3, company:"Décathlon", role:"Product Manager", date:"2025-03-12", scoreBefore:41, scoreAfter:91, keywords:["Product Roadmap","A/B Test","Analytics","Figma"] },
];
...
const Landing = ({ onLogin, onRegister }) => {
  ...
  const faqs = [
    ...
    ["Quels formats sont acceptés ?", "Import : PDF et DOCX. Export : PDF et DOCX, compatibles tous ATS et messageries professionnelles."],
  ];
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:24, maxWidth:680, margin:"0 auto" }}>
            ...
            <div style={{ ...s.card(34) }}>
              ...
              {[["check","1 optimisation par mois"],["check","Analyse ATS basique"],["check","Export avec filigrane"],["x","Optimisations illimitées"],["x","Export sans filigrane"],["x","Historique des versions"]].map(([t,l]) => (
                <div key={l} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10, fontSize:".875rem", color: t==="x" ? "var(--ink-mute)" : "var(--ink-soft)" }}>
    ...
    <div style={{ padding:36 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:20 }}>
        <div style={{ ...s.card(28) }}>
          ...
          <p style={{ fontSize:".875rem", color:"var(--ink-soft)", lineHeight:1.65, marginBottom:20 }}>Importez votre CV et collez une offre d'emploi pour obtenir une version optimisée en quelques secondes.</p>
        ...
        {selected && (
          <div style={{ ...s.card(24), animation:"slideIn .3s ease", alignSelf:"start", position:"sticky", top:20 }}>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button style={{ ...s.btn("primary","sm"), flex:1, justifyContent:"center" }}><Icon name="download" size={13}/> PDF</button>
              <button style={{ ...s.btn("outline","sm"), flex:1, justifyContent:"center" }}><Icon name="eye" size={13}/> Voir CV</button>
            </div>
          </div>
        )}
      </div>
...
const Settings = ({ user, onLogout }) => {
  ...
  const handleSave = () => { setSaved(true); setToast({ msg:"Modifications enregistrées !", type:"success" }); setTimeout(() => setSaved(false), 2000); };

  return (
    <div style={{ padding:36 }}>
      {toast && <Toast msg={toast.msg} type={toast.type} onDone={() => setToast(null)}/>}
      <div style={{ marginBottom:28 }}>
        <div style={{ fontFamily:"var(--fd)", fontSize:"1.8rem", fontWeight:700, marginBottom:4 }}>Paramètres</div>
        <p style={{ color:"var(--ink-mute)", fontSize:".9rem" }}>Gérez votre profil, votre abonnement et vos données.</p>
      </div>
